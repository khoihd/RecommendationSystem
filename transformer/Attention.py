...
class Attention(nn.Module):
    def __init__(self, input_dim, attn_dim, attn_head, mask):
        super().__init__()
    ...
    def forward(self, q, k, v):
        ...
        attn_q = (
            self.Q(q)
            .view(batch_size, query_len, self.attn_head, self.attn_dim)
            .transpose(1, 2)
        )
        attn_k = (
            self.K(k)
            .view(batch_size, keyvalue_len, self.attn_head, self.attn_dim)
            .transpose(1, 2)
        )
        attn_v = (
            self.V(v)
